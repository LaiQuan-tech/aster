...
-- 執行順序（四步，照順序做，不要跳）
-- ─────────────────────────────────────────────────────────────────
-- 步驟 1　先跑這一條，確認沒有重複的專案編號。有列出來就先處理，
--         否則 [3] 的唯一索引會建失敗，整批停在那裡。
--         （2026-09-13 起 [0] 會自動做同一件事並中止整批；這條留著給人先看。）
--
...
--           having count(*) > 1;
--
--         → No rows returned 才往下。
--
-- 步驟 2　把 [1] 到 [7] 整段貼進 SQL Editor 執行一次。
--         段與段之間有相依（[7] 要 [6] 的 contracts，[6] 要 [5] 的
--         project_settings），**不要只跑其中一段**。
--
-- 步驟 3　跑檔末「驗證」那一條 select（要把註解符號拿掉）。
--         ⚠️ SQL Editor 一次執行只顯示最後一條語句的結果，所以驗證
--         必須**單獨跑**，不要和步驟 2 接在一起貼。
--
-- 步驟 3b 接著套 docs/套用-2026-09-13-增量.sql（[8] 勞退自提率欄位，相依本檔 0 項，
--         但既然要開 SQL Editor 就一起跑）；驗證合併在 docs/驗證-2026-09-13-套用後.sql。
--
-- 步驟 4　[4] 會把舊的 status='archived' 一律轉成 'closed' 並標記
--         「實際案情待人工確認」——**舊模型沒記它是結案還是解約，
--         這一筆是猜的**。跑完把它們列出來補：
--
...
--
-- 另外 docs/驗證-trigger行為實測.sql 的 C、D 兩段（實測禁刪與稽核
-- trigger 真的有作用）與本檔獨立，那兩段也要分開跑。
-- =====================================================================

-- ─────────────────────────────────────────────────────────────────
-- [0] 前置檢查（2026-09-13 加）：步驟 1 的重複編號檢查改為自動擋
-- ─────────────────────────────────────────────────────────────────
-- 有重複的專案編號就在這裡中止，整批（同一次執行）一條都不會套上去，
-- 不會停在 [3] 的唯一索引留下半套的狀態。訊息裡會列出是哪些編號。
DO $$
DECLARE
  v_dups text;
BEGIN
  SELECT string_agg(tenant_id::text || ':' || code || '×' || cnt, ', ')
    INTO v_dups
    FROM (SELECT tenant_id, code, count(*) AS cnt
            FROM public.projects
           WHERE code IS NOT NULL
           GROUP BY tenant_id, code
          HAVING count(*) > 1) d;
  IF v_dups IS NOT NULL THEN
    RAISE EXCEPTION '專案編號重複，先處理再套用：%', v_dups;
  END IF;
END $$;
