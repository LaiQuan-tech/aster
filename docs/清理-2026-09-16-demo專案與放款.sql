...
-- 為什麼由業主自己跑：sql/0030 的 no_hard_delete_unless_draft 只允許刪「草稿」放款單的
-- 分攤列，demo 放款單有兩張是 paid → 要先在同一交易內把它們退回 draft 才刪得掉；
-- 這一步 AI 的安全規則判定為削弱保護而拒絕執行，所以請在 Supabase SQL Editor 貼上整段執行。
